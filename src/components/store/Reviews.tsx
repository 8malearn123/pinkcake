...
export function Reviews() {
  return (
    <section className="relative overflow-hidden border-y border-[#ddbd75]/30 bg-gradient-to-b from-[#7d2f49] via-[#9e3a5c] to-[#7d2f49] px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
      ...
      <div className="relative mx-auto max-w-[1500px]">
        <Reveal className="flex flex-col items-center text-center">
          <p className="flex items-center gap-2 text-xs font-bold tracking-[.14em] text-[#ddbd75]">
            <span className="h-px w-8 bg-[#ddbd75]/50" /> آراء عملائنا <span className="h-px w-8 bg-[#ddbd75]/50" />
          </p>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-[#ddbd75]">٤٫٩ / ٥ متوسط التقييم</span>
          </div>
        </Reveal>
        <Reveal className="reveal-grid mt-12 grid gap-5 md:grid-cols-3">
          {testimonials.map((t) => (
            <figure key={t.id} className="flex flex-col rounded-2xl bg-white p-7 shadow-[0_30px_60px_-30px_rgba(0,0,0,0.5)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_40px_80px_-32px_rgba(0,0,0,0.6)]">
              ...
              <blockquote className="mt-4 grow text-sm leading-7 text-[#4a3a40]">«{t.text}»</blockquote>
              <figcaption className="mt-5 border-t border-[#f3e8ec] pt-4">
                <p className="text-sm font-bold text-[#9e3a5c]">{t.name}</p>
                <p className="mt-0.5 text-xs text-[#8a827c]">{t.occasion}</p>
              </figcaption>
            </figure>
          ))}
        </Reveal>
