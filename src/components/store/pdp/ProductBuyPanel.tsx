  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {product.season && (
          <span className="rounded-full bg-seasonal/10 px-3 py-1 text-[11px] font-bold text-seasonal">
            🥭 تشكيلة {product.season}
          </span>
        )}
        <span className="flex items-center gap-1.5 text-[11px] font-bold text-muted-foreground">
          <span className="size-1.5 rounded-full bg-gold" /> تُخبز في جازان
        </span>
      </div>
      ...
      >
        ...
        {reviews > 0 && rating ? (
          ...
          <span className="text-muted-foreground underline decoration-gold decoration-2 underline-offset-4">
            كوني أول من يقيّمها
          </span>
        )}
      </button>
      ...
      {product.description && (
        <p className="mt-5 max-w-prose text-[15px] leading-8 text-muted-foreground">{product.description}</p>
      )}

      {/* Fading-gold hairline: story zone → buy zone (same cue as the cards) */}
      <div className="my-6 h-px bg-gradient-to-r from-gold/70 via-gold/25 to-transparent" />
      ...
      <p className="mt-3 flex items-center gap-2 text-[13px] font-bold">
        {soldOut ? (
          <span className="flex items-center gap-2 text-primary">
            <span className="size-2 rounded-full bg-primary" /> نفدت الكمية — جرّبي تشكيلتنا المشابهة أدناه
          </span>
        ) : lowStock ? (
          <span className="flex items-center gap-2 text-seasonal">
            <span className="size-2 animate-pulse rounded-full bg-seasonal" /> بقي {toArabicDigits(stock ?? 0)} قطع فقط لهذا اليوم
          </span>
        ) : (
          <span className="flex items-center gap-2 text-success">
            <span className="size-2 rounded-full bg-success" /> متوفّرة الآن · تُحضّر طازجة عند الطلب
          </span>
        )}
        <span className="text-muted-foreground">|</span>
        <span className="text-muted-foreground">تكفي ٨–١٢ شخصاً</span>
      </p>

      {/* Occasions — real catalogue tags, each one a way back into the shop */}
      {occasions.length > 0 && (
        <div className="mt-5">
          <p className="text-[11px] font-bold tracking-[.08em] text-rose">مناسبة لـ</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {occasions.map((o) => (
              <button
                ...
                className="rounded-full border border-primary/20 px-3.5 py-1.5 text-xs font-bold text-muted-foreground transition-colors hover:border-primary hover:bg-blush hover:text-primary"
              >
                {o}
              </button>
            ))}
      ...
      <div ref={ref} className="mt-7 flex flex-wrap items-center gap-3">
        ...
        {!soldOut && (
          <div className="flex h-[58px] w-full items-center justify-between rounded-2xl border-2 border-primary/15 bg-background px-1.5 sm:w-auto sm:justify-start">
            <button
              type="button"
              onClick={() => onQtyChange(Math.max(1, qty - 1))}
              disabled={qty <= 1}
              aria-label="إنقاص الكمية"
              className="grid size-11 place-items-center rounded-xl text-primary transition-colors hover:bg-blush disabled:opacity-35 disabled:hover:bg-transparent"
            >
            ...
            <button
              ...
              className="grid size-11 place-items-center rounded-xl text-primary transition-colors hover:bg-blush"
            >
        ...
        <button
          ...
          className={`flex h-[58px] min-w-[220px] flex-1 items-center justify-center gap-2.5 rounded-2xl text-base font-black text-white shadow-[0_16px_34px_-14px_hsl(var(--primary)/0.8)] transition-all duration-200 active:scale-[.98] ${
            soldOut
              ...
              : added
                ? 'bg-success'
                : 'bg-gradient-to-t from-pink-dark to-primary hover:from-primary hover:to-rose hover:shadow-[0_22px_40px_-14px_hsl(var(--rose)/0.85)]'
          }`}
        >
          {soldOut ? (
            'نفدت الكمية'
      ...
      {!soldOut && (
        <div className="mt-5 rounded-2xl border border-gold/40 bg-background p-4">
          <p className="flex items-center gap-2 text-[13px] font-bold text-gold-deep">
      ...
      <button
        type="button"
        onClick={onCustomize}
        className="group/custom mt-4 flex w-full items-center gap-3.5 rounded-2xl border border-primary/15 bg-gradient-to-l from-blush to-background p-4 text-start transition-colors hover:border-primary/40"
      >
        ...
        <ChevronLeft size={18} className="shrink-0 text-primary transition-transform duration-300 group-hover/custom:-translate-x-1" />
      </button>
    </div>
  );
});
