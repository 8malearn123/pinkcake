...
export function PairsWithRail({ items, inCart, onAdd, onView }: PairsWithRailProps) {
  ...
  return (
    <section className="rounded-3xl border border-border bg-background p-5 shadow-[0_18px_50px_-38px_hsl(var(--primary)/0.5)] sm:p-6">
      ...
      <ul className="mt-5 space-y-3">
        {items.map((item) => {
          ...
          return (
            <li
              key={item.id}
              className="flex items-center gap-3.5 rounded-2xl border border-secondary bg-background p-2.5 transition-colors hover:border-border"
            >
              <button
                type="button"
                onClick={() => onView(item)}
