...
export function StoreProductCard({
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-[#f3e8ec] bg-[#fffdfa] shadow-[0_12px_40px_-26px_rgba(158,58,92,0.4)] transition-all duration-300 hover:-translate-y-1.5 hover:border-[#e8d3db] hover:shadow-[0_26px_60px_-28px_rgba(158,58,92,0.55)]">
      {/* Media */}
      <div className="relative aspect-[4/5] overflow-hidden bg-[#fbeef2]">
        {product.image_url ? (
          ...
          <div className="grid size-full place-items-center bg-gradient-to-b from-[#fbeef2] to-[#f3e8ec]">
            <Cake size={56} strokeWidth={1.25} className="text-[#b0506e]/40" />
          </div>
        )}
        ...
        <div className="pointer-events-none absolute end-3 top-3 z-10 flex flex-col items-end gap-1.5">
          {discount > 0 && (
            <span className="rounded-full bg-[#9e3a5c] px-2.5 py-1 text-[11px] font-bold text-white shadow-md">
              خصم {toArabicDigits(discount)}٪
            </span>
          )}
          {product.season && (
            <span className="rounded-full bg-[#e8942f] px-2.5 py-1 text-[11px] font-bold text-white shadow-md">
              🥭 موسمي
            </span>
          )}
        </div>
        ...
        >
          <Heart
            ...
            className={`transition-all duration-300 ${
              isFavorite ? 'scale-110 fill-[#9e3a5c] text-[#9e3a5c]' : 'text-[#857077]'
            }`}
          />
        </button>
        ...
        {product.category && (
          <span className="pointer-events-none absolute bottom-3 start-3 z-10 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-bold text-[#2c2226] shadow-sm backdrop-blur">
            {product.category}
          </span>
        )}
        ...
        <button
          ...
          className={`pointer-events-none absolute inset-x-3 bottom-3 z-20 flex translate-y-3 items-center justify-center gap-1.5 rounded-full py-2.5 text-xs font-bold opacity-0 shadow-lg backdrop-blur transition-all duration-300 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100 ${
            inCart > 0 ? 'bg-[#2c7a5f] text-white' : 'bg-white/95 text-[#9e3a5c]'
          }`}
        >
          {inCart > 0 ? (
            <>
            ...
            <>
              <Zap size={14} className="fill-[#ddbd75] text-[#ddbd75]" /> أضف بسرعة
            </>
          )}
        </button>
      </div>
      ...
      <div className="flex grow flex-col p-4">
        ...
        <h3 className="text-base font-black leading-6 text-[#2c2226]">
          {onView ? (
            <button
              type="button"
              onClick={onView}
        ...
        {reviews > 0 && rating && (
          <div className="mt-1.5 flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-bold text-[#857077]">({toArabicDigits(reviews)})</span>
          </div>
        )}

        {product.description && (
          <p className="mt-1.5 line-clamp-2 min-h-[2.5rem] text-xs leading-5 text-[#7d6870]">
            {product.description}
          </p>
        )}
        ...
        <div className="relative z-10 mt-auto pt-3">
          ...
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
            <span className="flex items-baseline gap-1 text-2xl font-black leading-none text-[#9e3a5c]">
              {toArabicDigits(product.price)} <RiyalSymbol className="text-lg" />
            </span>
            {discount > 0 && (
              <span className="flex items-baseline gap-1 text-sm text-[#a49d97] line-through">
          ...
          {lowStock && (
            <p className="mt-2.5 inline-flex items-center gap-1 rounded-md bg-[#e8942f]/10 px-2 py-1 text-[11px] font-bold text-[#e8942f]">
              <Flame size={13} className="fill-[#e8942f]" /> بقي {toArabicDigits(product.stock ?? 0)} قطع فقط!
            </p>
          )}

          {/* CTA — the visual anchor */}
          {inCart > 0 ? (
            <div className="cz-step mt-3 flex h-[46px] items-center justify-between rounded-xl border-2 border-[#9e3a5c] bg-[#9e3a5c]/[0.06] px-2">
              <button
                type="button"
                onClick={onRemoveOne}
                aria-label={`إنقاص كمية ${product.name}`}
                className="grid size-9 place-items-center rounded-lg text-[#9e3a5c] transition-colors hover:bg-[#9e3a5c]/10 active:scale-95"
              >
                <Minus size={16} strokeWidth={2.5} />
              </button>
              <span className="text-sm font-bold text-[#9e3a5c]">
            ...
            <button
              ...
              className="mt-3 flex h-[46px] w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-t from-[#8a3251] to-[#9e3a5c] text-sm font-bold text-white shadow-[0_10px_24px_-10px_rgba(158,58,92,0.75)] transition-all duration-200 hover:from-[#9e3a5c] hover:to-[#b0506e] hover:shadow-[0_16px_30px_-10px_rgba(176,80,110,0.8)] active:scale-[.98]"
            >
